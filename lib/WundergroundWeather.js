...
WundergroundWeather.prototype = {
	...
	getCurrentWeather: function(callback) {
		...
		request.get(
			'http://api.wunderground.com/api/33c9addcc118e257/geolookup/conditions/forecast/lang:VU/q/Vietnam/Ho_Chi_Minh.json?hours=' + hour,
	...
	getHourlyWeather: function(callback) {
		...
		request.get(
			'http://api.wunderground.com/api/33c9addcc118e257/geolookup/conditions/hourly/lang:VU/q/Vietnam/Ho_Chi_Minh.json?hours=' + hour,
			function(error, response, body) {
				var jsonBody;
				if (!error && response.statusCode === 200) {
