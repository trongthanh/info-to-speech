...
const randomGossip = [
	...
	'Hôm nay chị Ba luôn nghĩ đến $name đó, ahihi.',
	'$name đừng quên nhập timesheet nhé.',
	'$name hãy cố lên nhé, chị Ba luôn ở đây.',
	'Chị Ba yêu mấy cưng.',
...
function composeInfoString(weather, clock) {
	...
	if (currentHour === 10) {
		text += 'Chị Ba chúc mấy cưng một ngày ' + clock.getWeekDay() + ' tuyệt vời, ahihi. ';
	} else if (currentHour === 12) {
		text += 'Đã đến giờ ăn trưa. ';
	} else if (currentHour === 17) {
		text += 'Ai chưa đóng tiền ăn thì đóng đi nhé, ahihi. ';
	} else if (currentHour === 18) {
		text += 'Đã đến giờ vào hồ bơi. Ra về nhớ đổ rác giúp chị Ba nhé, yêu lắm cơ. ';
	} else if (currentHour <= 19 && currentHour >= 7) {
		//
		var randText = randomGossip[getRandomInt(0, randomGossip.length - 1)];
		var randName = names[getRandomInt(0, names.length - 1)];

		randText = randText.replace('$name', randName);
	...
	if (nextHourCondition && nextHourCondition !== weather.mainCondition()) {
